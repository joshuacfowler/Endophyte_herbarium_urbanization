...
# Using TIN
s_components.TIN.year <-  ~ 0 +  fixed(main = ~ 0 + Spp_code*std_year*std_TIN + Spp_code*std_year*std_ag + Spp_code*std_year*std_urb, model = "fixed")+
...
s_components.TIN<-  ~ 0 +  fixed(main = ~ 0 + Spp_code*std_ag + Spp_code*std_urb + Spp_code*std_TIN, model = "fixed")+
  scorer(scorer_index, model = "iid", constr = TRUE, mapper = bru_mapper_index(max(data$scorer_index)), hyper = list(pc_prec)) +
  collector(collector_index, model = "iid", constr = TRUE, mapper = bru_mapper_index(max(data$collector_index, na.rm = T)), hyper = list(pc_prec))+
  space_int(coords, model = spde) 

# Using NO3
s_components.NO3.year <-  ~ 0 +  fixed(main = ~ 0 + Spp_code*std_year*std_NO3 + Spp_code*std_year*std_ag + Spp_code*std_year*std_urb, model = "fixed")+
  scorer(scorer_index, model = "iid", constr = TRUE, mapper = bru_mapper_index(max(data$scorer_index)), hyper = list(pc_prec)) +
  collector(collector_index, model = "iid", constr = TRUE, mapper = bru_mapper_index(max(data$collector_index, na.rm = T)), hyper = list(pc_prec))+
  space_int(coords, model = spde)


s_components.NO3<-  ~ 0 +  fixed(main = ~ 0 + Spp_code*std_ag + Spp_code*std_urb + Spp_code*std_NO3, model = "fixed")+
  scorer(scorer_index, model = "iid", constr = TRUE, mapper = bru_mapper_index(max(data$scorer_index)), hyper = list(pc_prec)) +
  collector(collector_index, model = "iid", constr = TRUE, mapper = bru_mapper_index(max(data$collector_index, na.rm = T)), hyper = list(pc_prec))+
  space_int(coords, model = spde) 

# Using NH4
s_components.NH4.year <-  ~ 0 +  fixed(main = ~ 0 + Spp_code*std_year*std_NH4 + Spp_code*std_year*std_ag + Spp_code*std_year*std_urb, model = "fixed")+
  scorer(scorer_index, model = "iid", constr = TRUE, mapper = bru_mapper_index(max(data$scorer_index)), hyper = list(pc_prec)) +
  collector(collector_index, model = "iid", constr = TRUE, mapper = bru_mapper_index(max(data$collector_index, na.rm = T)), hyper = list(pc_prec))+
  space_int(coords, model = spde)


s_components.NH4<-  ~ 0 +  fixed(main = ~ 0 + Spp_code*std_ag + Spp_code*std_urb + Spp_code*std_NH4, model = "fixed")+
  scorer(scorer_index, model = "iid", constr = TRUE, mapper = bru_mapper_index(max(data$scorer_index)), hyper = list(pc_prec)) +
  collector(collector_index, model = "iid", constr = TRUE, mapper = bru_mapper_index(max(data$collector_index, na.rm = T)), hyper = list(pc_prec))+
  space_int(coords, model = spde) 
...
fit.TIN.year <- bru(s_components.TIN.year,
             like(
               formula = s_formula,
               family = "binomial",
               Ntrials = 1,
...
fit.NO3.year <- bru(s_components.NO3.year,
                    like(
                      formula = s_formula,
                      family = "binomial",
                      Ntrials = 1,
...
fit.NH4.year <- bru(s_components.NH4.year,
                    like(
                      formula = s_formula,
                      family = "binomial",
                      Ntrials = 1,
...
fit.NO3$dic$dic
fit.NH4$dic$dic
fit.NO3.year$dic$dic
fit.NH4.year$dic$dic


fit.NO3$mode$mode.status # a 0 or low value indicates "convergence"
fit.NH4$mode$mode.status # a 0 or low value indicates "convergence"
fit.NO3.year$mode$mode.status # a 0 or low value indicates "convergence"
fit.NH4.year$mode$mode.status # a 0 or low value indicates "convergence"
...
preddata.3 <- tibble(Spp_code = c(rep("AGHY", times = 50),rep("AGPE",times = 50),rep("ELVI",times = 50)),
                     std_ag = 0,
                     std_urb = 0,
                     std_NO3 = rep(seq(min_nit, max_nit, length.out = 50)-mean_nit, times = 3),
                     collector_index = 9999, scorer_index = 9999) %>% 
  mutate(species = case_when(Spp_code == "AGHY" ~ species_names[1],
                             Spp_code == "AGPE" ~ species_names[2],
                             Spp_code == "ELVI" ~ species_names[3]))
...
nit.pred <- predict(
  ...
  probs = c(0.025, 0.25, 0.5, 0.75, 0.975),
  n.samples = 100) %>% 
  mutate(mean_NO3 = std_NO3 + mean_nit)
...
nit_trend <- ggplot(nit.pred) +
  geom_line(aes(mean_NO3, mean)) +
  geom_ribbon(aes(mean_NO3, ymin = q0.025, ymax = q0.975), alpha = 0.2, fill = "#BF00A0") +
  geom_ribbon(aes(mean_NO3, ymin = q0.25, ymax = q0.75), alpha = 0.2) +
  geom_point(data = nit_binned, aes(x = mean_nit, y = mean_endo, size = sample), color = "black", shape = 21)+
  scale_size_continuous(limits=c(1,260))+
  facet_wrap(~species, ncol = 1, scales = "free_x", strip.position="right")+  
  labs(y = "Endophyte Prevalence", x = "NO3 Deposition (kg N/km^2)", color = "Year", fill = "Year", shape = "Year", size = "Sample Size")+
  theme_classic()+
  theme(strip.background = element_blank(), 
...
ggsave(fig_NO3, file = "Figure_NO3.png", width = 10, height = 8)
...
param_names.TIN <- fit.TIN$summary.random$fixed$ID
param_names.NO3 <- fit.NO3$summary.random$fixed$ID
param_names.NH4 <- fit.NH4$summary.random$fixed$ID


# we can sample values from the join posteriors of the parameters with the addition of "_latent" to the parameter name
posteriors.TIN <- generate(
  fit.TIN,
  formula = ~ fixed_latent,
  n.samples = n_draws) 
rownames(posteriors.TIN) <- param_names.TIN
colnames(posteriors.TIN) <- c( paste0("iter",1:n_draws))


posteriors.NO3 <- generate(
  fit.NO3,
  formula = ~ fixed_latent,
  n.samples = n_draws) 
rownames(posteriors.NO3) <- param_names.NO3
colnames(posteriors.NO3) <- c( paste0("iter",1:n_draws))


posteriors.NH4 <- generate(
  fit.NH4,
  formula = ~ fixed_latent,
  n.samples = n_draws) 
rownames(posteriors.NH4) <- param_names.NH4
colnames(posteriors.NH4) <- c( paste0("iter",1:n_draws))

posteriors.TIN_df <- as_tibble(t(posteriors.TIN), rownames = "iteration")
posteriors.NO3_df <- as_tibble(t(posteriors.NO3), rownames = "iteration")
posteriors.NH4_df <- as_tibble(t(posteriors.NH4), rownames = "iteration")



# Calculate the effects of the predictor, given that the reference level is for AGHY
effects.TIN_df <- posteriors.TIN_df %>% 
...
effects.NO3_df <- posteriors.NO3_df %>% 
  mutate(NIT.AGHY = std_NO3,
         NIT.AGPE = std_NO3+`Spp_codeAGPE:std_NO3`,
         NIT.ELVI = std_NO3+`Spp_codeELVI:std_NO3`,
         AG.AGHY = std_ag,
         AG.AGPE = std_ag+`Spp_codeAGPE:std_ag`,
         AG.ELVI = std_ag+`Spp_codeELVI:std_ag`,
         URB.AGHY = std_urb,
...
effects.NH4_df <- posteriors.NH4_df %>% 
  mutate(NIT.AGHY = std_NH4,
         NIT.AGPE = std_NH4+`Spp_codeAGPE:std_NH4`,
         NIT.ELVI = std_NH4+`Spp_codeELVI:std_NH4`,
         AG.AGHY = std_ag,
         AG.AGPE = std_ag+`Spp_codeAGPE:std_ag`,
         AG.ELVI = std_ag+`Spp_codeELVI:std_ag`,
         URB.AGHY = std_urb,
...
effects_df <- bind_rows(effects.TIN_df, effects.NO3_df, effects.NH4_df)
...
posterior_hist <- ggplot(filter(effects_df))+
  ...
  facet_wrap(~ model+ param_f, scales = "free_x")+
...
ggsave(posterior_hist, filename = "Nitrogen_comparison_posterior_without_year.png", width = 14, height = 10)
...
summary_data <- data %>% 
  ...
  summarize(min_year = min(std_year),
            ...
            min_nit = quantile(std_NO3, .05),
            max_nit = quantile(std_NO3, .95),
            mean_nit = mean(std_NO3))
...
preddata_aghy <- expand.grid(Spp_code = c("AGHY"), 
                             ...
                             std_NO3 = c(summary_data[summary_data$Spp_code == "AGHY",]$min_nit, summary_data[summary_data$Spp_code == "AGHY",]$max_nit),
                             std_ag = 0,
                             std_urb = 0)
preddata_agpe <- expand.grid(Spp_code = c("AGPE"), 
                             ...
                             std_NO3 = c(summary_data[summary_data$Spp_code == "AGPE",]$min_nit, summary_data[summary_data$Spp_code == "AGPE",]$max_nit),
                             std_ag = 0,
                             std_urb = 0)
preddata_elvi <- expand.grid(Spp_code = c("ELVI"), 
                             ...
                             std_NO3 = c(summary_data[summary_data$Spp_code == "ELVI",]$min_nit, summary_data[summary_data$Spp_code == "ELVI",]$max_nit),
                             std_ag = 0,
                             std_urb = 0)
...
preddata <- bind_rows(preddata_aghy, preddata_agpe, preddata_elvi) %>% 
  mutate(collector_index = 9999, scorer_index = 9999,
         species = case_when(Spp_code == "AGHY" ~ species_names[1],
                             Spp_code == "AGPE" ~ species_names[2],
                             Spp_code == "ELVI" ~ species_names[3]),
         nit_label = case_when(std_NO3 > 0 ~ "High Nitrogen",
                               std_NO3 < 0 ~ "Low Nitrogen"))
...
param_names.TIN.year <- fit.TIN.year$summary.random$fixed$ID
param_names.NO3.year <- fit.NO3.year$summary.random$fixed$ID
param_names.NH4.year <- fit.NH4.year$summary.random$fixed$ID


# we can sample values from the join posteriors of the parameters with the addition of "_latent" to the parameter name
posteriors.TIN.year <- generate(
  fit.TIN.year,
  formula = ~ fixed_latent,
  n.samples = n_draws) 
rownames(posteriors.TIN.year) <- param_names.TIN.year
colnames(posteriors.TIN.year) <- c( paste0("iter",1:n_draws))


posteriors.NO3.year <- generate(
  fit.NO3.year,
  formula = ~ fixed_latent,
  n.samples = n_draws) 
rownames(posteriors.NO3.year) <- param_names.NO3.year
colnames(posteriors.NO3.year) <- c( paste0("iter",1:n_draws))


posteriors.NH4.year <- generate(
  fit.NH4.year,
  formula = ~ fixed_latent,
  n.samples = n_draws) 
rownames(posteriors.NH4.year) <- param_names.NH4.year
colnames(posteriors.NH4.year) <- c( paste0("iter",1:n_draws))

posteriors.TIN.year_df <- as_tibble(t(posteriors.TIN.year), rownames = "iteration")
posteriors.NO3.year_df <- as_tibble(t(posteriors.NO3.year), rownames = "iteration")
posteriors.NH4.year_df <- as_tibble(t(posteriors.NH4.year), rownames = "iteration")
...
# Calculate the effects of the predictor, given that the reference level is for AGHY
effects.TIN.year_df <- posteriors.TIN.year_df %>% 
  mutate(NIT.AGHY = std_TIN,
         NIT.AGPE = std_TIN+`Spp_codeAGPE:std_TIN`,
         NIT.ELVI = std_TIN+`Spp_codeELVI:std_TIN`,
         URB.AGHY = std_urb,
         ...
         INT.AGPE = Spp_codeAGPE,
         INT.ELVI = Spp_codeELVI) %>% 
  select(-all_of(param_names.TIN.year)) %>% 
  pivot_longer( cols = -c(iteration), names_to = "param") %>% 
  mutate(model = "TIN") %>% 
  mutate(spp_label = sub(".*\\.", "", param),
         param_label = sub("\\..*","", param)) %>% 
...
effects.NO3.year_df <- posteriors.NO3.year_df %>% 
  mutate(NIT.AGHY = std_NO3,
         NIT.AGPE = std_NO3+`Spp_codeAGPE:std_NO3`,
         NIT.ELVI = std_NO3+`Spp_codeELVI:std_NO3`,
         URB.AGHY = std_urb,
         ...
         YEAR.AGPE = std_year+`Spp_codeAGPE:std_year`,
         YEAR.ELVI = std_year+`Spp_codeELVI:std_year`,
         YEARxNIT.AGHY = `std_year:std_NO3`,
         YEARxNIT.AGPE = `std_year:std_NO3`+`Spp_codeAGPE:std_year:std_NO3`,
         YEARxNIT.ELVI = `std_year:std_NO3`+`Spp_codeELVI:std_year:std_NO3`,
         YEARxURB.AGHY = `std_year:std_urb`,
         YEARxURB.AGPE = `std_year:std_urb`+`Spp_codeAGPE:std_year:std_urb`,
         YEARxURB.ELVI = `std_year:std_urb`+`Spp_codeELVI:std_year:std_urb`,
         YEARxAG.AGHY = `std_year:std_ag`,
         ...
         INT.AGPE = Spp_codeAGPE,
         INT.ELVI = Spp_codeELVI) %>% 
  select(-all_of(param_names.NO3.year)) %>% 
  pivot_longer( cols = -c(iteration), names_to = "param") %>% 
  mutate(model = "NO3") %>% 
  mutate(spp_label = sub(".*\\.", "", param),
         param_label = sub("\\..*","", param)) %>% 
...
effects.NH4.year_df <- posteriors.NH4.year_df %>% 
  mutate(NIT.AGHY = std_NH4,
         NIT.AGPE = std_NH4+`Spp_codeAGPE:std_NH4`,
         NIT.ELVI = std_NH4+`Spp_codeELVI:std_NH4`,
         URB.AGHY = std_urb,
         ...
         YEAR.AGPE = std_year+`Spp_codeAGPE:std_year`,
         YEAR.ELVI = std_year+`Spp_codeELVI:std_year`,
         YEARxNIT.AGHY = `std_year:std_NH4`,
         YEARxNIT.AGPE = `std_year:std_NH4`+`Spp_codeAGPE:std_year:std_NH4`,
         YEARxNIT.ELVI = `std_year:std_NH4`+`Spp_codeELVI:std_year:std_NH4`,
         YEARxURB.AGHY = `std_year:std_urb`,
         YEARxURB.AGPE = `std_year:std_urb`+`Spp_codeAGPE:std_year:std_urb`,
         YEARxURB.ELVI = `std_year:std_urb`+`Spp_codeELVI:std_year:std_urb`,
         YEARxAG.AGHY = `std_year:std_ag`,
         ...
         INT.AGPE = Spp_codeAGPE,
         INT.ELVI = Spp_codeELVI) %>% 
  select(-all_of(param_names.NH4.year)) %>% 
  pivot_longer( cols = -c(iteration), names_to = "param") %>% 
  mutate(model = "NH4") %>% 
  mutate(spp_label = sub(".*\\.", "", param),
         param_label = sub("\\..*","", param)) %>% 
...
effects.year_df <- bind_rows(effects.TIN.year_df, effects.NO3.year_df, effects.NH4.year_df)
...
posterior_hist <- ggplot(effects.year_df)+
  stat_halfeye(aes(x = value, y = spp_f, fill = spp_label), breaks = 50, normalize = "panels", alpha = .6)+
  # stat_histinterval(aes(x = value, y = spp_label, fill = spp_label), breaks = 50, alpha = .6)+
  # geom_point(data = posteriors_summary, aes(x = mean, y = spp_label, color = spp_label))+
  # geom_linerange(data = posteriors_summary, aes(xmin = lwr, xmax = upr, y = spp_label, color = spp_label))+
  ...
  geom_vline(xintercept = 0)+
  facet_wrap(~ model + param_f, scales = "free_x", nrow = 3)+
  scale_color_manual(values = species_colors)+
  scale_fill_manual(values = species_colors)+
  labs(x = "Posterior Est.", y = "Species")+
  guides(fill = "none")+
...
posterior_hist <- tag_facet2(posterior_hist)
ggsave(posterior_hist, filename = "Nitrogen_posterior_comparison_with_year.png", width = 12, height = 7)

...
data_summary <- endo_herb %>% 
  dplyr::summarize(year = mean(year, na.rm = T),
                   ...
                   mean_NO3_10km = mean(mean_NO3_10km, na.rm = T),
                   mean_NH4_10km = mean(mean_NH4_10km, na.rm = T),
...
data <- endo_herb %>% 
  mutate(year = year - data_summary$year,
         ...
         mean_NO3_10km = mean_NO3_10km - data_summary$mean_NO3_10km,
         mean_NH4_10km = mean_NH4_10km - data_summary$mean_NH4_10km,
...
fit.year.TIN <- bru(s_components.year.TIN,
                like(
                  formula = s_formula,
                  family = "binomial",
                  Ntrials = 1,
...
fit.year.NO3 <- bru(s_components.year.NO3,
                    like(
                      formula = s_formula,
                      family = "binomial",
                      Ntrials = 1,
...
fit.year.NH4 <- bru(s_components.year.NH4,
                    like(
                      formula = s_formula,
                      family = "binomial",
                      Ntrials = 1,
...
fit.year.TIN$dic$dic
fit.year.NO3$dic$dic
fit.year.NH4$dic$dic


# WAIC
fit.TIN$waic$waic
fit.year.TIN$waic$waic


# checking convergence
fit.TIN$mode$mode.status
fit.NO3$mode$mode.status
fit.NH4$mode$mode.status

fit.year.TIN$mode$mode.status
fit.year.NO3$mode$mode.status
fit.year.NH4$mode$mode.status
...
colnames(posteriors.TIN_df) <- sub("Spp_code", "", colnames(posteriors.TIN_df))
colnames(posteriors.TIN_df) <- gsub(":", ".", colnames(posteriors.TIN_df))

posteriors.NO3_df <- as_tibble(t(posteriors.NO3), rownames = "iteration")
colnames(posteriors.NO3_df) <- sub("Spp_code", "", colnames(posteriors.NO3_df))
colnames(posteriors.NO3_df) <- gsub(":", ".", colnames(posteriors.NO3_df))

posteriors.NH4_df <- as_tibble(t(posteriors.NH4), rownames = "iteration")
colnames(posteriors.NH4_df) <- sub("Spp_code", "", colnames(posteriors.NH4_df))
colnames(posteriors.NH4_df) <- gsub(":", ".", colnames(posteriors.NH4_df))



# Calculate the effects of the predictor, given that the reference level is for AGHY
effects.TIN <- posteriors.TIN_df %>% 
...
effects_df <- bind_rows(effects.TIN, effects.NO3, effects.NH4)

N_colors <- c("#808080", "#FFC0CB", "#FF00FF")
...
# we can sample values from the join posteriors of the parameters with the addition of "_latent" to the parameter name
posteriors.year.TIN <- generate(
  fit.year.TIN,
  formula = ~ fixed_latent,
  n.samples = n_draws) 
rownames(posteriors.year.TIN) <- param_names.year.TIN
colnames(posteriors.year.TIN) <- c( paste0("iter",1:n_draws))

posteriors.year.NO3 <- generate(
  fit.year.NO3,
  formula = ~ fixed_latent,
  n.samples = n_draws) 
rownames(posteriors.year.NO3) <- param_names.year.NO3
colnames(posteriors.year.NO3) <- c( paste0("iter",1:n_draws))

posteriors.year.NH4 <- generate(
  fit.year.NH4,
  formula = ~ fixed_latent,
  n.samples = n_draws) 
rownames(posteriors.year.NH4) <- param_names.year.NH4
colnames(posteriors.year.NH4) <- c( paste0("iter",1:n_draws))


posteriors.year.TIN_df <- as_tibble(t(posteriors.year.TIN), rownames = "iteration")
colnames(posteriors.year.TIN_df) <- sub("Spp_code", "", colnames(posteriors.year.TIN_df))
colnames(posteriors.year.TIN_df) <- gsub(":", ".", colnames(posteriors.year.TIN_df))

posteriors.year.NO3_df <- as_tibble(t(posteriors.year.NO3), rownames = "iteration")
colnames(posteriors.year.NO3_df) <- sub("Spp_code", "", colnames(posteriors.year.NO3_df))
colnames(posteriors.year.NO3_df) <- gsub(":", ".", colnames(posteriors.year.NO3_df))

posteriors.year.NH4_df <- as_tibble(t(posteriors.year.NH4), rownames = "iteration")
colnames(posteriors.year.NH4_df) <- sub("Spp_code", "", colnames(posteriors.year.NH4_df))
colnames(posteriors.year.NH4_df) <- gsub(":", ".", colnames(posteriors.year.NH4_df))


# Calculate the effects of the predictor, given that the reference level is for AGHY
effects.year.TIN <- posteriors.year.TIN_df %>% 
  rename(AGHY.Int = AGHY, AGPE.Int = AGPE, ELVI.Int = ELVI) %>% 
  pivot_longer( cols = -c(iteration), names_to = "param") %>% 
  mutate(model = "Year",
         data = "Total N") %>% 
...
effects.year.NO3 <- posteriors.year.NO3_df %>% 
  rename(AGHY.Int = AGHY, AGPE.Int = AGPE, ELVI.Int = ELVI) %>% 
  pivot_longer( cols = -c(iteration), names_to = "param") %>% 
  mutate(model = "Year",
         data = "NO3") %>% 
...
effects.year.NH4 <- posteriors.year.NH4_df %>% 
  rename(AGHY.Int = AGHY, AGPE.Int = AGPE, ELVI.Int = ELVI) %>% 
  pivot_longer( cols = -c(iteration), names_to = "param") %>% 
  mutate(model = "Year",
         data = "NH4") %>% 
...
effects_df <- bind_rows(effects.year.TIN, effects.year.NO3, effects.year.NH4)
